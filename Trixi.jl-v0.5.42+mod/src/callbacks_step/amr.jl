...
#=
# mutable version for counting RHS
mutable struct AMRCallback{Controller, Adaptor, Cache}
    controller::Controller
    interval::Int
    adapt_initial_condition::Bool
    adapt_initial_condition_only_refine::Bool
    ...
    adaptor::Adaptor
    amr_cache::Cache
    # For counting the scalar RHS calls: interval * length(u_ode)
    rhs_calls::Int
end
=#
...
function (amr_callback::AMRCallback)(integrator; kwargs...)
    ...
    @trixi_timeit timer() "AMR" begin
        ...
        #=
        amr_callback.rhs_calls += amr_callback.interval * length(u_ode)
        println(amr_callback.rhs_calls)
        =#
    end
...
# Quick & Dirty enabler for enstrophy-based AMR for 3D TGV
function (amr_callback::AMRCallback)(u_ode::AbstractVector, mesh::TreeMesh,
                                     equations::CompressibleEulerEquations3D, dg::DG,
                                     cache, cache_parabolic,
                                     semi::SemidiscretizationHyperbolicParabolic,
                                     t, iter;
                                     only_refine = false, only_coarsen = false)
